...
function StatCard({ stat, accent }: StatCardProps) {
  const { ref, isHovering } = use3DTilt<HTMLDivElement>({
    max: 8,
    perspective: 1200,
    scale: 1.03,
    speed: 400,
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `radial-gradient(circle at top, ${accent}33, transparent 65%)`,
          ...
          <div className="grid w-full max-w-3xl grid-cols-1 gap-3 sm:grid-cols-3">
            {persona!.stats.map((stat, index) => (
              <FloatingElement
                ...
                yOffset={8}
              >
                <StatCard stat={stat} accent={persona!.accent} />
              </FloatingElement>
            ))}
          </div>
        </div>

...
export function LaunchCountdown() {
  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-[32px] border border-white/10 bg-white/[0.03] p-8 md:p-12 backdrop-blur-2xl">
        <div className="flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
          <div className="space-y-3">
            ...
            <p className="text-sm text-zinc-300/80 md:text-base">
          ...
          <div className="grid flex-1 grid-cols-2 gap-4 sm:grid-cols-4">
            {units.map((unit) => (
              <div
                key={unit.label}
                className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/60 px-4 py-5 text-center shadow-[0_18px_40px_rgba(7,7,12,0.45)]"
              >
        ...
        <div className="mt-10 grid gap-4 md:mt-12 md:grid-cols-3">
          {siteConfig.launch.milestones.map((milestone) => (
            <div
              key={milestone.title}
              className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02] p-5 backdrop-blur-xl transition-transform duration-300 hover:-translate-y-1"
            >
